...
const FindingBadge = ({ status }: { status: AssessmentObjective['status'] }) => {
    switch(status) {
        ...
        default: return <span className="bg-coral-100 text-coral-600 px-2 py-0.5 rounded text-[10px] font-black uppercase tracking-widest border border-coral-200 flex items-center gap-1"><Clock size={10}/> PENDING</span>;
    }
};
  ...
  return (
    <div className="max-w-7xl mx-auto p-8 space-y-10 h-full flex flex-col bg-slate-50/30">
      ...
      <div className="bg-white rounded-3xl p-10 border border-slate-200 shadow-sm flex flex-col md:flex-row justify-between items-center gap-8">
          <div className="flex items-center gap-8">
              <div className="w-24 h-24 bg-slate-900 rounded-[2rem] flex items-center justify-center text-white shadow-2xl rotate-3 shrink-0">
                  <ClipboardCheck size={48} className="text-coral-500" />
              </div>
              <div>
                  <h1 className="text-4xl font-black text-slate-900 tracking-tighter uppercase leading-none">
                    Assessor Gateway
                    <div className="mt-2 flex items-center gap-3">
                        <span className="bg-coral-600 text-white text-[10px] px-3 py-1 rounded-full tracking-widest font-black uppercase shadow-lg shadow-coral-200">CMMC CAP V2.0</span>
                        <span className="text-slate-300 text-lg font-medium tracking-widest">//</span>
                        <span className="text-coral-600 text-[10px] tracking-widest font-black uppercase">Official Portal</span>
                    </div>
                  </h1>
          ...
          <div className="flex flex-col gap-4">
            <div className="flex gap-3">
                 <button onClick={() => setShowInviteModal(true)} className="flex items-center gap-3 bg-coral-600 text-white px-8 py-3.5 rounded-2xl font-black text-xs shadow-2xl shadow-coral-200 hover:bg-coral-700 transition-all uppercase tracking-[0.1em]">
              ...
              return (
                  <button 
                    ...
                    className={`text-left p-6 rounded-[2rem] border-2 transition-all flex flex-col h-full relative group ${
                        isActive ? 'bg-white border-coral-600 shadow-2xl ring-8 ring-coral-50' : 'bg-white border-slate-100 hover:border-slate-300 hover:shadow-lg'
                    }`}
                  >
                      <div className={`p-4 rounded-2xl w-fit mb-4 transition-all ${isActive ? 'bg-coral-600 text-white shadow-xl shadow-coral-200 scale-110' : 'bg-slate-50 text-slate-400 border border-slate-200 group-hover:bg-coral-50'}`}>
                          <phase.icon size={24} />
                      </div>
                      <h4 className={`text-sm font-black uppercase tracking-widest ${isActive ? 'text-coral-600' : 'text-slate-500'}`}>{phase.label}</h4>
                      <p className="text-[11px] text-slate-400 font-medium mt-2 leading-relaxed">{phase.description}</p>
                      {isActive && <div className="absolute top-6 right-6 text-coral-600 animate-bounce"><ArrowRight size={20}/></div>}
                  </button>
              );
          })}
      </div>
          ...
          {activeCapPhase === 'PH1_PRE_ASSESSMENT' && (
              <div className="flex-1 flex flex-col animate-in fade-in slide-in-from-bottom-4 duration-500">
                  ...
                  <div className="p-12 pb-8 bg-slate-50/50 border-b border-slate-100 relative overflow-hidden">
                      <div className="absolute top-0 right-0 w-96 h-96 bg-coral-600/5 rounded-full blur-3xl -mr-48 -mt-48"></div>
                  ...
                  <div className="flex-1 p-12 overflow-y-auto">
                      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
                          ...
                          {/* Tile: SSP Review */}
                          <div className="bg-white rounded-[2.5rem] border border-slate-200 shadow-sm hover:shadow-xl transition-all p-10 flex flex-col h-full group border-b-8 border-b-coral-600">
                              <div className="p-5 bg-coral-50 text-coral-600 rounded-3xl w-fit mb-8 group-hover:scale-110 transition-transform">
                                  <FileCheck size={32} />
                              </div>
                              ...
                              <div className="space-y-4">
                                  <div className="bg-slate-50 p-5 rounded-2xl border border-slate-100 flex items-center justify-between">
                                      <div className="flex items-center gap-3">
                                          <FileText className="text-coral-600" size={20} />
                                          <span className="text-sm font-bold text-slate-700 tracking-tight">SSP_v2.4_Production.pdf</span>
                                      </div>
                                      <button className="text-coral-600 font-black text-xs uppercase hover:underline flex items-center gap-1">Open <ExternalLink size={12}/></button>
                                  </div>
                          ...
                          <div className="bg-white rounded-[2.5rem] border border-slate-200 shadow-sm hover:shadow-xl transition-all p-10 flex flex-col h-full group border-b-8 border-b-coral-600">
                              ...
                              <button className="w-full py-4 bg-coral-600 text-white rounded-2xl font-black text-xs uppercase tracking-[0.2em] shadow-xl hover:bg-coral-700 transition-all">Confirm Boundary</button>
                          </div>
                      ...
                      <div className="mt-16 bg-slate-900 rounded-[2.5rem] p-12 text-center relative overflow-hidden shadow-2xl">
                          <div className="absolute top-0 left-0 w-full h-full bg-coral-600/10"></div>
                          <div className="relative z-10 max-w-2xl mx-auto">
                              <h3 className="text-3xl font-black text-white uppercase tracking-tighter mb-4">Final Readiness Determination</h3>
                              <p className="text-coral-100 font-medium mb-8 text-lg">
                                If all Pre-Assessment criteria are MET, the Lead Assessor may issue a "Go" decision to move to active Fieldwork (Phase 2).
                              </p>
                              <button className="bg-coral-600 hover:bg-coral-500 text-white px-12 py-5 rounded-[2rem] font-black text-sm uppercase tracking-[0.3em] shadow-2xl shadow-coral-900/50 transition-all flex items-center gap-4 mx-auto hover:scale-105 active:scale-95">
          ...
          {activeCapPhase === 'PH2_ASSESSMENT' && (
              <>                   <div className="p-8 border-b border-slate-100 bg-slate-50/50 flex flex-col md:flex-row justify-between items-center gap-6 shrink-0">
                      <div className="flex-1 relative max-w-2xl">
                          ...
                          <input 
                            className="w-full pl-16 pr-6 py-5 bg-white border border-slate-200 rounded-3xl text-lg focus:ring-4 focus:ring-coral-500/10 focus:border-coral-500 outline-none shadow-inner font-bold"
                      ...
                      <div className="flex gap-4">
                           <div className="bg-coral-50 text-coral-700 px-6 py-3 rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] flex items-center gap-3 border border-coral-100">
                  ...
                  <div className="flex-1 overflow-y-auto min-0 custom-scrollbar">
                      <div className="divide-y divide-slate-100">
                          {filteredReqs.map(req => {
                              ...
                              return (
                                  <div key={req.id} className="group">                                       <div onClick={() => setExpandedReqId(isExpanded ? null : req.id)} className={`p-8 flex items-center justify-between cursor-pointer transition-colors ${isExpanded ? 'bg-coral-50/30' : 'hover:bg-slate-50'}`}>
                                          <div className="flex items-center gap-8">
                                              <div className={`w-20 font-mono text-sm font-black transition-colors ${isExpanded ? 'text-coral-600' : 'text-slate-400'}`}>{req.id}</div>
                                              <div>
                                              ...
                                              <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
                                                  <div className="lg:col-span-8 space-y-8">
                                                      <div className="bg-white rounded-[2.5rem] border border-slate-200 shadow-sm overflow-hidden">
                                                          <div className="bg-slate-900 px-8 py-4 text-white text-[11px] font-black uppercase tracking-[0.2em] flex items-center gap-3">
                                                              <SearchCode size={18} className="text-coral-400" /> Assessment Methodology Guide
                                                          </div>
                                                          <div className="p-8 grid grid-cols-1 md:grid-cols-3 gap-8">
                                                              <div className="space-y-4">
                                                                  <h5 className="text-[11px] font-black text-coral-600 uppercase tracking-widest flex items-center gap-2"><FileText size={16}/> Examine</h5>
                                                                  <ul className="space-y-2">
                                                                      {(req.examineOptions || ['Policies', 'Configuration Settings', 'Audit Logs']).map((opt, i) => (
                                                                          <li key={i} className="text-xs text-slate-600 font-medium leading-relaxed flex gap-2">
                                                                              <span className="text-coral-400 font-black">•</span> {opt}
                                                                          </li>
                                                                      ))}
                                                                  </ul>
                                                              </div>
                                                              ...
                                                              <div className="space-y-4">
                                                                  <h5 className="text-[11px] font-black text-coral-600 uppercase tracking-widest flex items-center gap-2"><ShieldCheck size={16}/> Test</h5>
                                                                  <ul className="space-y-2">
                                                      ...
                                                      <div className="bg-slate-900 rounded-[2.5rem] p-10 text-white shadow-2xl relative overflow-hidden">
                                                          ...
                                                          <h4 className="text-[11px] font-black text-coral-300 uppercase tracking-[0.3em] mb-6 flex items-center gap-3"><BookOpen size={20}/> Organization implementation statement</h4>
                                                          <p className="text-lg font-medium leading-relaxed text-coral-50 italic">
                                                      ...
                                                      {req.policyMapping && (
                                                          <div className="bg-white rounded-[2.5rem] p-10 border border-slate-200 shadow-sm">
                                                              ...
                                                              <div className="flex items-center gap-4 mb-4">
                                                                  <div className="p-3 bg-coral-50 text-coral-600 rounded-xl"><BookOpen size={20}/></div>
                                                                  <div>
                                                                      ...
                                                                      <div className="text-[9px] text-coral-600 font-bold uppercase tracking-widest mt-1">
                                                  ...
                                                  <div className="lg:col-span-4 space-y-10">
                                                          ...
                                                          {reqArtifacts.length === 0 ? (
                                                              ...
                                                              <div className="space-y-3">
                                                                  {reqArtifacts.map(art => (
                                                                      <button key={art.id} className="w-full bg-white p-4 rounded-2xl border border-slate-200 flex items-center justify-between group/art hover:border-coral-400 hover:shadow-xl transition-all shadow-sm">
                                                                          <div className="flex items-center gap-4 overflow-hidden">
                                                                              <div className="p-3 bg-coral-50 text-coral-600 rounded-xl group-hover/art:bg-coral-600 group-hover/art:text-white transition-colors"><Link2 size={20}/></div>
                                                                          ...
                                                                          <Download size={18} className="text-slate-300 group-hover/art:text-coral-600 shrink-0" />
                                                                      </button>
                                                                  ))}
                                                              </div>
                                                          )}
                                                      ...
                                                      <div className="bg-white p-10 rounded-[2.5rem] border-2 border-slate-100 shadow-xl space-y-6">
                                                          ...
                                                          <textarea 
                                                            className="w-full h-32 bg-slate-50 border border-slate-200 rounded-2xl p-5 text-sm font-medium focus:ring-4 focus:ring-coral-500/10 focus:border-coral-600 outline-none transition-all resize-none shadow-inner"
          ...
          {activeCapPhase === 'PH3_REPORTING' && (
              <div className="flex-1 flex flex-col items-center justify-center p-20 text-center space-y-12 animate-in fade-in duration-500">
                  ...
                  <div className="flex justify-center gap-6">
                      <button className="bg-coral-600 text-white px-12 py-5 rounded-[2rem] font-black text-xs uppercase tracking-[0.2em] shadow-2xl shadow-coral-100 hover:bg-coral-700 transition-all hover:scale-105 active:scale-95">Generate Out-Brief Report</button>
                      <button className="bg-slate-900 text-white px-12 py-5 rounded-[2rem] font-black text-xs uppercase tracking-[0.2em] shadow-2xl shadow-slate-900/50 hover:bg-black flex items-center gap-3 transition-all hover:scale-105 active:scale-95">
                        <ArrowRight size={20} className="text-coral-400" /> Initiate eMASS Data Transfer
                      </button>
                  </div>
              </div>
          )}
          ...
          {activeCapPhase === 'PH4_CERTIFICATION' && (
              <div className="flex-1 flex flex-col items-center justify-center p-20 text-center space-y-12 animate-in fade-in zoom-in-95 duration-700">
                  ...
                  <div className="bg-slate-900 p-12 rounded-[3.5rem] border border-slate-800 inline-block max-w-xl text-white shadow-[0_35px_60px_-15px_rgba(0,0,0,0.3)] relative overflow-hidden">
                      ...
                      <div className="text-[11px] font-black text-coral-400 uppercase tracking-[0.4em] mb-8">Certificate Preparation Engine</div>
                      <div className="text-left space-y-4 mb-10">
                          ...
                          <div className="flex justify-between text-sm border-b border-white/5 pb-2"><span className="text-white/40 uppercase font-black tracking-widest text-[9px]">Global Unique ID:</span> <span className="font-mono font-bold text-coral-200">CMMC-2026-AX-PR-9921-X5</span></div>
                      </div>
      ...
      <div className="flex justify-between items-center px-6 pt-6 border-t-2 border-slate-200 text-[11px] font-black text-slate-400 uppercase tracking-[0.3em] shrink-0">
          ...
          <div className="text-coral-600">Cuallee Cyber Compliance Architecture V2.4.1</div>
      </div>

      {/* Invite Modal */}
      {showInviteModal && (
          <div className="fixed inset-0 bg-black/80 z-[200] flex items-center justify-center p-6 backdrop-blur-sm">
              <div className="bg-white rounded-[3rem] shadow-2xl w-full max-w-xl overflow-hidden animate-in fade-in zoom-in duration-300 border-t-[12px] border-coral-600">
                  <div className="bg-slate-900 p-10 flex justify-between items-center text-white">
                      <h3 className="font-black uppercase tracking-[0.2em] text-xl flex items-center gap-4">
                          <UserPlus size={28} className="text-coral-400" /> Grant Agency Access
                      </h3>
                  ...
                  <form onSubmit={handleInviteAuditor} className="p-12 space-y-10">
                      <div className="bg-coral-50 border border-coral-100 p-6 rounded-3xl flex items-start gap-4">
                          <Info className="text-coral-600 shrink-0" size={24} />
                          <p className="text-sm text-coral-900 leading-relaxed font-medium">Inviting an agency email will grant a 72-hour **Read-Only** assessment token. This action is logged for audit integrity.</p>
                      </div>
                      <div className="space-y-4">
                          ...
                          <div className="relative">
                              ...
                              <input 
                                type="email" 
                                className="w-full pl-16 pr-6 py-5 bg-slate-50 border-2 border-slate-100 rounded-[2rem] focus:ring-8 focus:ring-coral-500/10 focus:border-coral-600 outline-none transition-all font-bold text-lg text-slate-900" 
                      ...
                      <button type="submit" className="w-full py-6 bg-coral-600 hover:bg-coral-700 text-white rounded-[2rem] font-black uppercase tracking-[0.3em] text-sm shadow-2xl shadow-coral-200 transition-all hover:scale-105 active:scale-95">Deploy Session Token</button>
                  </form>
              </div>
          </div>
      )}
