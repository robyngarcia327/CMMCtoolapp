...
export const Dashboard: React.FC<DashboardProps> = ({ 
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto space-y-8 overflow-y-auto h-full">
        ...
        <div className="bg-white rounded-3xl p-8 border border-silver-200 shadow-sm flex flex-col md:flex-row justify-between items-center gap-8 relative overflow-hidden">
             ...
             <div className="flex items-center gap-6 relative z-10">
                <div className="w-16 h-16 bg-slate-900 rounded-2xl flex items-center justify-center text-white shadow-xl rotate-3 group hover:rotate-0 transition-transform">
                    <Target size={32} className="text-coral-500" />
                </div>
                <div>
                    <h1 className="text-3xl font-black text-slate-900 tracking-tight uppercase">Mission Control</h1>
                    <p className="text-slate-500 font-medium flex items-center gap-2">
                        Scoped Level: <span className="bg-coral-100 text-coral-700 px-2 py-0.5 rounded text-[10px] font-black uppercase tracking-widest border border-coral-200">CMMC Level {targetLevel}</span>
                    </p>
                </div>
             </div>
                    ...
                    {[1, 2, 3].map((lvl) => (
                        <button 
                            ...
                            className={`px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${
                                targetLevel === lvl ? 'bg-white shadow text-coral-600' : 'text-slate-400 hover:text-slate-600'
                            }`}
                        >
                            Level {lvl}
                        </button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            <button 
              ...
              className="bg-white p-6 rounded-2xl shadow-sm border border-silver-200 text-left relative overflow-hidden group hover:border-coral-300 transition-all"
            >
                <h3 className="text-slate-400 font-bold text-[10px] uppercase tracking-widest mb-2">Posture Score</h3>
                <div className="text-5xl font-black text-slate-900 group-hover:text-coral-600 transition-colors">{complianceScore}%</div>
                <div className="w-full bg-slate-100 h-2 mt-4 rounded-full overflow-hidden">
                    <div className="bg-coral-500 h-full transition-all duration-1000" style={{ width: `${complianceScore}%` }}></div>
                </div>
            </button>

            <button 
              onClick={() => onNavigate(AppView.CONTROLS)}
              className="bg-white p-6 rounded-2xl shadow-sm border border-silver-200 text-left hover:border-green-300 transition-all group"
            >
            ...
            <button 
              onClick={() => onNavigate(AppView.CONTROLS)}
              className="bg-white p-6 rounded-2xl shadow-sm border border-silver-200 text-left hover:border-red-300 transition-all group"
            >
            ...
            <button 
              ...
              className="bg-white p-6 rounded-2xl shadow-sm border border-silver-200 text-left hover:border-purple-300 transition-all group"
            >
                <h3 className="text-slate-400 font-bold text-[10px] uppercase tracking-widest mb-2">Evidence Repository</h3>
                <div className="text-4xl font-black text-coral-600">{artifacts.length}</div>
        ...
        {roadmap && (
            <div className="bg-white border-2 border-coral-100 rounded-3xl p-8 animate-in fade-in slide-in-from-top-4 duration-500 relative shadow-xl">
                ...
                <div className="flex items-center gap-4 mb-6">
                    <div className="bg-coral-600 p-3 rounded-2xl text-white shadow-lg shadow-coral-200"><Map size={24} /></div>
                    <div>
                        <h2 className="text-xl font-black text-slate-900 leading-tight">Remediation Roadmap</h2>
                        <p className="text-coral-600 font-bold text-xs uppercase tracking-widest">AI Strategic Priority Outline</p>
                    </div>
                </div>
        ...
        <div className="flex justify-center">
            ...
            >
                {isGeneratingRoadmap ? <Loader2 size={18} className="animate-spin text-coral-500" /> : <Sparkles size={18} className="text-coral-500" />}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            >
                <div>
                    <h3 className="text-xl font-black mb-2 flex items-center gap-2">Assessment Hub <ChevronRight className="text-coral-500 group-hover:translate-x-1 transition-transform" /></h3>
                ...
                <div className="text-xs font-bold text-coral-400 tracking-widest uppercase flex items-center gap-2">
                    Start Remediation <ChevronRight size={14} />
                </div>
            </button>

            <div className="bg-white border border-silver-200 rounded-[2.5rem] p-8 flex flex-col justify-between h-56 shadow-sm">
                <div>
