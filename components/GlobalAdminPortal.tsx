      ...
      <div className="bg-slate-900 rounded-3xl p-8 text-white shadow-2xl relative overflow-hidden">
          ...
          <div className="relative z-10 flex flex-col md:flex-row justify-between items-center gap-6">
              <div className="flex items-center gap-6">
                  <div className="w-16 h-16 bg-blue-500 rounded-2xl flex items-center justify-center shadow-lg rotate-3">
                      <Shield size={32} />
                  </div>
                  <div>
                      <h1 className="text-3xl font-black tracking-tight uppercase">Global Admin Command</h1>
                      <p className="text-blue-300 font-bold text-xs uppercase tracking-[0.2em] mt-1 flex items-center gap-2">
              ...
              <div className="flex gap-4">
                  <div className="bg-white/10 px-6 py-3 rounded-2xl border border-white/10 backdrop-blur-md">
                      <div className="text-[10px] font-black text-blue-300 uppercase tracking-widest mb-1">Active Tenants</div>
                      <div className="text-2xl font-black">{tenants.length}</div>
                  </div>
                  <div className="bg-white/10 px-6 py-3 rounded-2xl border border-white/10 backdrop-blur-md">
                      <div className="text-[10px] font-black text-blue-300 uppercase tracking-widest mb-1">Total Users</div>
                          ...
                          <tr key={tenant.id} className="hover:bg-slate-50 group">
                              <td className="p-5">
                                  <div className="flex items-center gap-4">
                                      <div className="w-10 h-10 bg-slate-100 rounded-xl flex items-center justify-center text-slate-400 font-black text-sm border border-slate-200 group-hover:bg-blue-600 group-hover:text-white transition-colors">
                                          {tenant.logoInitial}
                                      </div>
                                      <div>
                          ...
                          <tr key={user.id} className="hover:bg-slate-50 group">
                              ...
                              <td className="p-5">
                                  <div className="flex items-center gap-2">
                                      <div className={`w-2 h-2 rounded-full ${
                                          ...
                                          user.role === 'Tenant_Admin' ? 'bg-blue-500' :
                              ...
                              <td className="p-5">
                                  <span className={`px-2 py-0.5 rounded text-[9px] font-black uppercase border ${
                                      user.role === 'Tenant_Admin' ? 'bg-indigo-50 text-indigo-700 border-indigo-200' : 'bg-slate-50 text-slate-400 border-slate-200'
                                  }`}>
                              ...
                              <td className="p-5 text-right">
                                  <select 
                                    className="bg-white border border-slate-200 rounded-lg text-[10px] font-black uppercase px-2 py-1 outline-none hover:border-blue-500"
                                    value={user.role}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                      {auditors.map(auditor => (
                          <div key={auditor.id} className="bg-slate-50 border border-slate-200 rounded-3xl p-6 relative group overflow-hidden">
                              ...
                              <button className="w-full mt-6 bg-white border border-slate-200 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-blue-600 hover:text-white transition-all">
                                  View Audit Assignments
                              </button>
                          </div>
                      ))}
                      <button className="border-4 border-dashed border-slate-100 rounded-3xl p-12 flex flex-col items-center justify-center text-slate-300 hover:border-blue-100 hover:text-blue-300 transition-all group">
